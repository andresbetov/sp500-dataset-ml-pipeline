...
def phase_3_cross_validation() -> dict:
    ...
    # Extract regime from metadata (for regime distribution analysis)
    regime = metadata.get("regime", None)
    if regime is None:
        logger.warning("regime not found in metadata, regime distribution will not be calculated")
    ...
    for fold_idx, (train_indices_sorted, test_indices_sorted) in enumerate(tscv.split(dates_sorted)):
        ...
        # Calculate regime distribution if regime metadata is available
        regime_dist_train = {}
        regime_dist_test = {}
        if regime is not None:
            regimes_train = regime[train_indices_original]
            regimes_test = regime[test_indices_original]
            for r in range(5):
                regime_dist_train[r] = int(np.sum(regimes_train == r))
                regime_dist_test[r] = int(np.sum(regimes_test == r))
        ...
        folds[fold_key] = {
            ...
            "regime_distribution_test": regime_dist_test,
        }
